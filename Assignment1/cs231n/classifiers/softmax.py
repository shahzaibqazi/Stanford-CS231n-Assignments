...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    score_func = lambda k: np.exp(scores[k]) / sum_scores
    loss += -np.log(score_func(y[i]))
    loss /= num_train
    loss += 0.5 * reg * np.sum(W * W)
    
    for j in range(num_classes):
        score_j = score_func(j)
        dW[:, j] += (score_j - (j == y[i])) * X[i]
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  score_func = lambda k: np.divide(np.exp(scores[:,k]), sum_scores)
  
  prob = np.exp(scores)/sum_scores
  loss = np.sum(-np.log(prob[range(num_train), y]))
  loss /= num_train
  loss += 0.5 * reg * np.sum(W * W)
  
  idx = np.zeros_like(prob)
